...
void LongJump::onEnable() {
	...
	case 2: // Hive
		hiveSpeedIndex = 125;
		hiveTick = 0;
        if (waitdamage) clientMessageF("WaitingDamage");
		break;
	}
}
...
void LongJump::onMove(C_MoveInputHandler* input) {
    ...
    case 2: // Hive
        ...
        if (waitdamage && !player->damageTime && getdamaged == false) {
            MoveUtil::stop(false);
            return;
        }
        ...
        else if (hiveTick >= 17 && waitdamage == false) setEnabled(false);
