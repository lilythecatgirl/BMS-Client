...
std::wstring SettingManager::GetRoamingFolderPath() {
	std::string SettingsFolder = (getenv("AppData") + (std::string)"\\..\\Local\\Packages\\Microsoft.MinecraftUWP_8wekyb3d8bbwe\\RoamingState\\PacketSkid\\");
...
void SettingManager::loadSettings(std::string name, bool create) {
	...
	if (SettingsExists || create) {
		...
		if (SettingsExists) {
			...
			currentSettingsObj["from"] = "PacketSkid";
		}
